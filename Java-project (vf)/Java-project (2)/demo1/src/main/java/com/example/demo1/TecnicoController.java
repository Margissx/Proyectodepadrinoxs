...
public class TecnicoController implements Initializable {
    ...
    public  void uploadFile(String filePath) throws Exception {
        ...
        try (InputStream in = new FileInputStream(filePath)) {
            String delimitador = "\\\\";
            FileMetadata metadata = client.files().uploadBuilder("/"+ carta.split(delimitador)[carta.split(delimitador).length-1])
                    .uploadAndFinish(in);
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("El archivo se subio");
            alert.setHeaderText(null);
            alert.setContentText("El archivo se subio");
            alert.showAndWait();
    ...
    public  void selectcarta(){
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Se enviara la carta seleccionada");
        alert.setHeaderText(null);
        alert.setContentText("Seleccionaste la carta "+carta);
        alert.showAndWait();
        UpdateStadoCarta();
    }
    @Override
    ...
    public  void LoadDataCartas(){
        ...
        try{
            ...
            String query = "select * from cartas where id_estado=2;";
    ...
    public void UpdateStadoCarta(){
        ...
        try{
            // Crear un objeto Statement para ejecutar consultas SQL
            String query = "UPDATE cartas SET id_estado = 1 WHERE hash=?;";
            ...
            alert.setHeaderText(null);
            alert.setContentText("Se esta enviando la carta");
            alert.showAndWait();
            try {
                uploadFile(carta);
